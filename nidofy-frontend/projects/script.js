...
window.addEventListener("DOMContentLoaded", async function () {
    if(localStorage.getItem("error")) {
        ...
        showTimedAlert(`Failed to open project: ${error}`);
        localStorage.removeItem("error");
    }
    
    let projectsList = await getListOfProjects();
    if("error" in projectsList) {
        showTimedAlert(`Failed to load project: ${projectsList.error}`);
        return;
    }

    projectsList = sortByDateAndId(projectsList);
    ...
    document.getElementById("gallery").innerHTML = `
        ...
        ${projectsList.map((project, i) => {
            ...
            <div class="col d-flex">
                <div class="mb-4 w-100" style="break-inside: avoid; max-width: 300px;">
